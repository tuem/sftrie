...
void exec(const std::string& corpus_path, const std::string& index_type, int max_result,
	...
	sftrie::lookup_table_mode lut_mode, float min_lookup_table_density, int min_binary_search)
{
	...
	if(index_type == "set" && optimization_mode == "original"){
		...
		sftrie::set_fast<text, integer> index(
			std::begin(texts), std::end(texts), two_pass, lut_mode, min_lookup_table_density, min_binary_search);
		history.record("construction", texts.size());
		std::cerr << "done." << std::endl;
	...
	try{
		...
		if(symbol_type == "char")
			exec<std::string, item, integer>(corpus_path, index_type, max_result,
				optimization_mode, two_pass, lut_mode, min_lookup_table_density, min_binary_search);
